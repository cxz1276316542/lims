...
<!--          placeholder="请输入状态 0-默认1-拟稿2-审核3-签发4-打印、邮寄"-->
<!--          clearable-->
<!--          size="small"-->
<!--          @keyup.enter.native="handleQuery"-->
<!--        />-->
...
<!--      </el-col>-->
    ...
    <el-table  v-loading="loading" :data="reportList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" :selectable="isChecked"/>
<!--      <el-table-column label="检测报告ID" align="center" prop="reportID" />-->
      ...
      <el-table-column sortable label="状态" align="center" prop="reportStatus" >
        <template slot-scope="scope">
          <span v-if="scope.row.reportStatus === 1">待签发</span>
          <span v-if="scope.row.reportStatus === 3">签发成功</span>
          <span v-if="scope.row.reportStatus === 4">签发失败</span>
        </template>
      </el-table-column>
...
<!--    </el-dialog>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="报告编号" prop="reportNumber">
          <el-input disabled v-model="form.reportNumber" placeholder="请输入报告编号" />
        </el-form-item>
        <el-form-item label="检品编号" prop="sampleNumber">
          <el-input disabled v-model="form.sampleNumber" placeholder="检品编号" />
        </el-form-item>
        <el-form-item label="样品学名" prop="reportNumber">
          <el-input disabled v-model="form.sampleAcademicName" placeholder="样品学名" />
        </el-form-item>
        <el-form-item label="检测标准" prop="standard">
          <el-input disabled v-model="form.standard" placeholder="检测标准" />
        </el-form-item>
        <el-form-item label="拒绝签发意见" prop="remark">
          <el-input v-model="form.remark" placeholder="请输入拒绝签发意见" required="required" />
        </el-form-item>
      </el-form>
      <el-form ref="form" :model="form" >
        <el-form-item label=""  prop="" label-width="100px">
          <el-radio-group v-model="check">
            ...
            <el-radio  label="2" >打回撰写</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
      <el-form ref="form" :model="form">
        <div class="demo-image__error" align="center">
          <div class="block">
            <div style="width: 200px; height: 200px; border:2px dotted #b4bccc ">
              <b>二维码</b>
            </div>
          </div>
        </div>
...
<script>
import { listReport, getReport, delReport, addReport, updateReport, exportReport,reportPass,reportFail } from "@/api/detection/report";
...
export default {
  ...
  //     // 嵌入页面URL
  //     type: String,
  //     default: 'http://localhost:8080/profile/upload/ZK2021081201.pdf',
  //   },
  // },
  data() {
    return {
      returnFlag:false,
      check:'1',
      // pdfsrc:'http://localhost:8080/profile/upload/ZK2021081201.pdf',
      pdfsrc:'',
      currentPage: 0, // pdf文件页码
      pageCount: 0, // pdf文件总页数
      fileType: 'pdf', // 文件类型
      // src: 'http://localhost:8080/profile/upload/ZK2021081201.pdf', // pdf文件地址
      ...
      rules: {
        ...
        remark: [
          { required: true, message: "拒绝签发意见不能为空!", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    reportPass() {
      let data = this.form
      this.$confirm('是否确认签发该报告？', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return reportPass(data);
      }).then(response => {
    ...
    reportFail() {
      ...
      this.$refs["form"].validate(valid => {
        ...
        debugger
        if(data.remark===null){
          this.msgError("拒绝签发理由不能为空！");
        }else{
        this.$confirm('是否确认该报告打回撰写？', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return  reportFail(data);
        }).then(response => {
          if (response.code === 200){
    ...
    handleReview(row) {
      if(row.reportStatus==1){
        this.reset();
        this.form.reportNumber = row.reportNumber;
        this.form.sampleNumber = row.sampleNumber;
        this.form.standard = row.standard;
        this.form.sampleAcademicName = row.sampleAcademicName;
        this.open = true;
        this.title = "签发";
      }else{
        this.msgError("请勿重复签发");
      }
