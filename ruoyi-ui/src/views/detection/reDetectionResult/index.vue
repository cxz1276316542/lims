...
<!--      <el-table-column label="项目检测数据记录ID" align="center" prop="detectionResultID" />-->
      ...
      <el-table-column label="审核状态" align="center" prop="auditStatus" >
        <template slot-scope="scope">
          <span v-if="scope.row.auditStatus === 2">待检</span>
          <span v-if="scope.row.auditStatus === 4">复检</span>
        </template>
      </el-table-column>
...
<script>
import {getReDetectionResult, listReDetectionResult, getDetectionResult, delDetectionResult, addDetectionResult, updateDetectionResult, exportDetectionResult ,submitReReview,submitBatchReReview} from "@/api/detection/detectionResult";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listReDetectionResult(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      if (row.auditStatus === 2) {
        ...
        getReDetectionResult(detectionResultID).then(response => {
    ...
    submitBatchReview() {
      ...
      for (let i = 0; i <items.length ; i++) {
          if(this.items[i].auditStatus!==2) flag=1;
      }
      if(flag===0&&items){
        console.log(items)
        this.$confirm('是否确认批量提交选中的检测结果?', "警告", {
          ...
          function() {
          return submitBatchReReview(items);
        }).then(() => {
          this.getList();
