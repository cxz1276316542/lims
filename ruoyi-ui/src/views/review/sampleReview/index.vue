...
<!--      </el-form-item>-->
      ...
      <el-form-item label="审核类别" prop="reviewCategory">
        <el-select v-model="queryParams.reviewCategory" placeholder="请选择审核类别" clearable filterable size="medium">
          <el-option
        ...
        <el-form-item label="登记人" prop="registrantID">
          <el-select v-model="queryParams.registrantID" placeholder="请选择登记人" clearable size="medium">
            <el-option
...
<!--      <el-table-column label="审核ID" align="center" prop="reviewID" />-->
      ...
      <el-table-column label="委托登记人" align="center" prop="registrantName" />
      <el-table-column label="审核类别" align="center" prop="reviewCategory" >
      <template slot-scope="scope">
      <span v-if="scope.row.reviewCategory == 11">初审</span>
      <span v-else-if="scope.row.reviewCategory == 12">复审</span>
       </template>
      </el-table-column>
      ...
      <el-table-column label="流转状态" align="center" prop="circulation" >
        <template slot-scope="scope">
          <el-tag size="mini" v-if="scope.row.circulation == 1">登记修改</el-tag>
          <el-tag size="mini" v-else-if="scope.row.circulation == 2">委托修改</el-tag>
          <el-tag size="mini" v-else-if="scope.row.circulation == 3">重新送样</el-tag>
...
<!--未审核表格信息-->
    <el-table v-loading="loading" v-if="radio ==='1'"  :data="reviewList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="提交时间" align="center" prop="09=" width="150" sortable>
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.operationTime, '{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="检品学名" align="center" prop="sampleScientificName" >
        <template slot-scope="scope">
          <span>{{ scope.row.sampleScientificName }}</span>
        </template>
        </el-table-column>
      <el-table-column label="检测标准" align="center" prop="subject.detectionStandard" />
      <el-table-column label="要求描述" align="center" prop="subject.sampleDescription" />
          ...
          <el-form ref="form" :model="form" :rules="rules" label-width="100px">
            ...
            <el-form-item label=""  prop="" label-width="100px">
              <el-radio-group v-model="check">
                ...
                <el-radio  label="2" >退回登记修改</el-radio>
                <el-radio  label="3" >退回委托修改</el-radio>
                <el-radio  label="4" >退回重新送样</el-radio>
              </el-radio-group>
            </el-form-item>
            </el-form>
...
import {
  ...
  exportReview,
  failReturnToModify,
  failReturnToCommissionModify,
  batchFailReview,
  batchReturnToModify,
  batchReturnToCommissionModify,
  getUserElectronicSignature, uploadSignature, getReviewInfo, userList
...
import {uploadSampleImage} from "@/api/commission/commissionPrototype";
...
export default {
  ...
  created() {
    ...
    // this.ele = this.$store.state.user;
  ...
  methods: {
    ...
    batchSubmitCheck(){
      if(this.batchCheck === '1'){
        ...
        this.batchReturnToModify();
      }else if(this.batchCheck === '3'){
        this.batchReturnToCommissionModify();
      }else if(this.batchCheck === '4'){
        this.batchFailReview();
      }
    },
    //审核通过点击事件
    passReview(){
    ...
    returnToModify(){
      this.$refs["form"].validate(valid => {
        let data =  this.form
        this.$confirm('是否确认该条数据退回登记修改？', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return failReturnToModify(data);
        }).then(response => {
          if (response.code === 200){
            this.msgSuccess("提交审核成功");
          }else{
    ...
    /** 审核不通过,退回委托修改*/
    returnToCommissionModify(){
      this.$refs["form"].validate(valid => {
        let data =  this.form
          this.$confirm('是否确认该条数据退回委托修改？', "警告", {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
            type: "warning"
          }).then(function() {
            return failReturnToCommissionModify(data);
          }).then(response => {
          if (response.code === 200){
            this.msgSuccess("提交审核成功");
          }else{
    ...
    failReview(){
      this.$refs["form"].validate(valid => {
        let data =  this.form
        this.$confirm('是否确认该条数据退回重新送样？', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
        return failReview(data);
        }).then(response => {
          if (response.code == 200){
            this.msgSuccess("提交审核成功");
          }else{
    ...
    handleReviewSearch(row) {
      ...
      getReviewed(reviewID).then(response => {
        ...
        if(this.form.reviewCategory === 11){
          this.form.reviewCategory = '初审'
        }else if(this.form.reviewCategory === 12){
    ...
    handleSearch(row) {
      this.reset();
      const reviewID = row.reviewID || this.ids
      sampleInfo(reviewID).then(response => {
    ...
    batchFailReview(){
      const ids = this.ids
      const data = this.form
      this.$confirm('是否确认批量退回重新送样？', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return batchFailReview(ids, data);
      }).then(response => {
        if (response.code == 200){
          this.msgSuccess("提交审核成功");
        }else{
    ...
    batchReturnToModify(){
      const ids = this.ids
      const data = this.form
      this.$confirm('是否确认批量退回登记修改？', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return batchReturnToModify(ids, data);
      }).then(response => {
        if (response.code == 200){
          this.msgSuccess("提交审核成功");
        }else{
    ...
    batchReturnToCommissionModify(){
      const ids = this.ids
      const data = this.form
      this.$confirm('是否确认批量退回委托修改？', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return batchReturnToCommissionModify(ids, data);
      }).then(response => {
        if (response.code == 200){
          this.msgSuccess("提交审核成功");
        }else{
