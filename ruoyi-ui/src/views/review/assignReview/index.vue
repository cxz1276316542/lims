<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch"  label-width="150px">
      ...
      <el-form-item label="分派人" prop="assignorID">
        <el-select v-model="queryParams.assignorID" placeholder="请选择分派人" clearable size="medium">
          <el-option
      ...
      <el-form-item label="" prop="" >
        ...
        <el-form-item label="提交/审核时间" prop="operationTimeList">
...
<!--已审核信息-->
    <el-table v-loading="loading" v-if="radio =='2'" :data="reviewList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分派人" align="center" prop="assignorName" />
      <el-table-column label="审核类别" align="center" prop="reviewCategory" >
        <template slot-scope="scope">
          <span v-if="scope.row.reviewCategory == 21">初审</span>
          <span v-else-if="scope.row.reviewCategory == 22">复审</span>
        </template>
      </el-table-column>
    ...
    <el-table v-loading="loading" v-if="radio ==='1'"  :data="reviewList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <!--      <el-table-column label="审核ID" align="center" prop="reviewID" />-->
      <el-table-column label="检品编号" alig  n="center" prop="sampleNumber" />
      <el-table-column label="分派人" align="center" prop="assignorName" />
      <el-table-column label="审核类别" align="center" prop="reviewCategory" >
        <template slot-scope="scope">
          <span v-if="scope.row.reviewCategory == 21">初审</span>
      ...
      <el-table-column label="检测方法" align="center" prop="subject.detectionMethodName" />
      <el-table-column label="检测员" align="center" prop="subject.inspectorName" sortable/>
      <el-table-column label="详细信息" align="center" class-name="small-padding fixed-width" >
        <template slot-scope="scope">
          <el-button
            size="mini"
          ...
          <el-form ref="form" :model="form" >
            <el-form-item label=""  prop="" label-width="100px">
              <el-radio-group v-model="check">
                ...
                <el-radio  label="2" >不 通 过</el-radio>
      ...
      <div v-else-if="title === '任务单分派详情'">
        ...
        <el-dialog :title="title"  :visible.sync="open" width="500px" append-to-body >
          <el-form ref="form" :model="form"  label-width="120px" class="inputDeep">
            ...
            <el-form-item label="检测方法" prop="detectionMethodName">
              <el-input v-model="form.detectionMethodName" style="width: 220px" size="medium" />
            </el-form-item>
...
import {
  ...
  updateReview,
  exportReview,
  pass, fail, assignInfo, notReviewList, batchPass, batchFailAssignReview
} from "@/api/review/assignReview";
import {getSampleScientificNameList} from "@/api/stock/sampleBank";
import {batchReturnToRecheck} from "@/api/review/reportReview";
import {getReviewInfo, getUserElectronicSignature, uploadSignature, userList} from "@/api/review/sampleReview";
  ...
  export default {
    ...
    methods: {
      ...
      getNoReviewList() {
        ...
        console.log(this.queryParams)
        notReviewList(this.queryParams).then(response => {
      ...
      handleSearch(row) {
        ...
        assignInfo(reviewID).then(response => {
      ...
      /** 批量分派审核不通过,退回重新分派*/
      batchFailAssignReview(){
        const ids = this.ids
        const data = this.form
        console.log(data)
        this.$confirm('是否确认批量提交审核不通过，退回重新分派？', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return batchFailAssignReview(ids, data);
        }).then(response => {
          if (response.code === 200){
            this.msgSuccess("提交审核成功");
          }else{
